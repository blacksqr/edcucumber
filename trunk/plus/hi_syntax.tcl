...
proc ifInWord {id} {
    ...
    if [fake compare $id == "$id linestart"] {
	if [regexp {\W} [fake get $id]] {
	    return 0
	}
	return -1
    }

    if [fake compare $id == "$id lineend"] {
	if [regexp {\W} [fake get "$id - 1c"]] {
	    return 0
	}
	return 1
    }
    
    set p [fake get "$id -1c"]
    set i [fake get $id]
    set s "$p$i"
...
proc indexWordHead {id} {
    if [fake compare $id == "$id linestart"] {
	return [fake index $id]
    } else {
	set i 1
	while {![regexp {\W} [fake get "$id - $i c"]]} {
	    if [fake compare "$id - $i c" == "$id linestart"] {
		return [fake index {insert linestart}]
	    }
	    incr i
	}
	return [fake index "$id - $i c + 1c"]
    }
}
...
proc indexWordEnd {id} {
	...
	while {![regexp {\W} [fake get "$id + $i c"]]} {
	    incr i
	    if [fake compare "$id + $i c" == "$id lineend"] {
		break
	    }
	}
	return [fake index "$id + $i c"]
    }
}
